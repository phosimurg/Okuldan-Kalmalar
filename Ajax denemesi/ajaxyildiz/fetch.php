...
foreach($result as $row)
{
 ...
 for($count=1; $count<=5; $count++)
 {
  ...
  $output .= '<li title="'.$count.'" id="'.$row['id'].'-'.$count.'" data-index="'.$count.'"  data-business_id="'.$row['id'].'" data-rating="'.$rating.'" class="rating" style="cursor:pointer; '.$color.' font-size:30px;">&#9733;</li>';
 }
 $output .= '
 </ul>
 <p>'.$row["address"].'</p>
